...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer,self).__init__(inputPlayerId, "TD-Learning - Smaller State Space")
    ...
    def updateMemory(self, compressedState):
        ...
        compressedStateHash = compressedState.__hash__()

        # if the current state is not in memory, add it to memory, and give the utility the reward value of the state
        if compressedStateHash not in self.stateUtilityMemory:
            currentReward = self.getReward(compressedState)
            self.stateUtilityMemory[compressedStateHash] = [currentReward, currentReward, INIT_ET]

        # get the values stored for current state -
        currentStateUtilAndEt = self.stateUtilityMemory[compressedStateHash]
        ...
        if self.PreviousState is None:
            ...
            delta = self.getReward(self.PreviousState) + self.gamma * currentStateUtilAndEt[UTILITY_INDEX] - \
                    self.stateUtilityMemory[prevStateHash][UTILITY_INDEX]

            # update the previous State's ET to 1
            self.stateUtilityMemory[prevStateHash][ET_INDEX] = INIT_ET
    ...
    def consolidateState(self, currentState):
        ...
        myAntCoords = [self.generalizeCoords(ant.coords) for ant in myInventory.ants if ant.type == WORKER]

        for coords in myAntCoords:
            compressedState.workerPositionList.append(coords)

        # Enemy Queen Location
        enemyQueen = enemyInventory.getQueen()
        if(enemyQueen is not None):
            compressedState.enemyQueenLocation = self.generalizeCoords(enemyQueen.coords)
            compressedState.enemyQueenHealth = enemyQueen.health
        else:
            compressedState.enemyQueenLocation = None
            compressedState.enemyQueenHealth = None
...
class state:
    ...
    ## TODO ##
    def __init__(self):
        self.workerPositionList = []
        #self.antPositionList = []
        self.enemyQueenLocation = None
        self.enemyQueenHealth = None
    ...
    def __eq__(self, other):
        if set(self.workerPositionList) == set(other.workerPositionList) and \
            self.enemyQueenLocation == other.enemyQueenLocation and \
            self.enemyQueenHealth == other.enemyQueenHealth and \
    ...
    def __hash__(self):
        return hash((tuple(self.workerPositionList), 
                     self.enemyQueenLocation, 
                     self.enemyQueenHealth,
                     self.hasWon, 
                     self.hasLost))
...
class UnitTests:
    ...
    def testCompressState(self):
        ...
        expectedCompressedState.workerPositionList = [(0,3)]

        expectedCompressedState.enemyQueenLocation = (0,3)
        expectedCompressedState.enemyQueenHealth = 1

        expectedCompressedState.hasWon = False
    ...
    def putOurInventory(self, inventory):
        ...
        inventory.ants.append(Ant((0,6), WORKER, PLAYER_ONE)) # Queen
        inventory.foodCount = 2
